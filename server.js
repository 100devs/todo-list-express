...
const PORT = 2121; //setting a constant to define the location where oir server will be listening
...
app.get("/", async (request, response) => {
  ...
  response.render("index.ejs", { items: todoItems, left: itemsLeft }); //rendering the EJS file and passing through the db items and the count remaining inide of an object
...
app.put("/markComplete", (request, response) => {
  ...
  db.collection("todos")
    .updateOne(
      ...
      {
        //look in the db for one item matching the nane of the item passed in from the main.js file that was clicked on
        $set: {
  ...
  db.collection("todos")
    .updateOne(
      { thing: request.body.itemFromJS },
      {
        ...
        $set: {
          completed: false, //set completed statis to false
        },
      },
      {
...
app.delete("/deleteItem", (request, response) => {
  ...
  db.collection("todos")
    ...
    .then((result) => {
      ...
      console.log("Todo Deleted"); //logging successful collection
      response.json("Todo Deleted"); //sending respose back to the sender
    })
    .catch((error) => console.error(error)); //catching errors
}); //ending delete
