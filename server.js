...
const PORT = 2121 // setting a constant to define the location where our sever will be listening
...
let db, // declare a variable called db but not assign a value 
    ...
    dbName = 'todo' // declaring a variable and assgining the name of the database we will be using
 
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) //creating a connection to MongoDB and passing in our connection string. Also passes in an additional property 
...
app.post('/addTodo', (request, response) => {
    db.collection('todos').insertOne({thing: request.body.todoItem, completed: false}) // inserts a new items into todos collection, gives it a completed value of false by default 
...
app.delete('/deleteItem', (request, response) => { // starts a delete method when the delete route is passed 
    db.collection('todos').deleteOne({thing: request.body.itemFromJS}) // look inside the todos collection for the ONE item has a matching name from our JS file
