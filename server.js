...
//create a connection to MongoDB, pass in connection string and additional property
...
app.post('/addTodo', (request, response) => { //start a POST method when the "addTodo" route is passed in, set up request and response parameters
    ...
    .then(result => { //if insert is successful, do tasks below
        console.log('Todo Added') //console log action
        response.redirect('/') //redirect back to the root route (homepage)
    }) //close .then
    .catch(error => console.error(error)) //catch errors and put them in console
}) //end POST method
...
app.listen(process.env.PORT || PORT, ()=>{ //set up port to listen, either the port from the .env file or the port variable set on server.js file
