...
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) //creating a connection to MongoDB, and passing in our connection string. Also passing in an additional property
    .then(client => { //waiting for the connection and proceeding if successful, and passing in all the client information
        ...
        db = client.db(dbName) //db variable reassigned that contains a db client factory method
    }) //closing our .then
...
app.put('/markComplete', (request, response) => { //starts a PUT method when the markComplete route is passed in
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{ //look in the db for one item matching the name of the itme passed in from the main.js file
        $set: {
...
app.delete('/deleteItem', (request, response) => { //starts a delete method when the delete route is passed
    db.collection('todos').deleteOne({thing: request.body.itemFromJS}) //look inside the todos collection for hte ONE item that has a matching name from our JS file
    .then(result => {
...
app.listen(process.env.PORT || PORT, ()=>{ //setting up which port we should be listening on - either the port from the .env file or the port variable we set
