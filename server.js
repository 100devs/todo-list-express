// assign the express variable to the imported module express
...
// We are declaring three variables here: db, dbConnectionStr and dbName and initializing two of them.
// dbConnectionStr holds the value of the environment variable we set up in .env as DB_STRING
...
// We are passing the useUnifiedTopology (deprecated, it's default as t5rue in newer versions of mongo) and setting it as true
...
app.get('/',async (request, response)=>{
    // access collection called 'todos' from the connected database and find all documents, as an array, then await the promise, assigning the documents to todoItems
    const todoItems = await db.collection('todos').find().toArray()
    // access collection called 'todos' from connected database and get the count of all documents that match the filter - have a property of completed with a value of false, awaiting this promise and assigning to itemsLeft
...
app.post('/addTodo', (request, response) => {
    ...
    // once the previous process is completed, we console log a message and redirect to /
    .then(result => {
...
app.put('/markUnComplete', (request, response) => {
    // selecting the 'todos' collection of our db and updating one item(object: key= thing, value= itemFromJs)
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{
        // using the mongo set operator, setting the completed key to false
        $set: {
            completed: false
          }
    },{
        // mongo sort method, sort in descending order (latest first)
        sort: {_id: -1},
        // setting the upsert (insert + update) mongo method to false (which is the default value)
        upsert: false
    })
    // when promise is completed, console log the 'marked complete' and log the same as json
    .then(result => {
        console.log('Marked Complete')
        response.json('Marked Complete')
    })
    // if the promise fails, console log error
    .catch(error => console.error(error))
