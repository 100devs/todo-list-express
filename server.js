...
app.use(express.json()); //parse json from incoming data

app.get("/", async (request, response) => {
  //Makes a get for the root path
  const todoItems = await db.collection("todos").find().toArray(); //Grab the todo list items from the todos collection
  const itemsLeft = await db
    .collection("todos")
    .countDocuments({ completed: false }); //Count the todos that are not completed
...
app.post("/addTodo", (request, response) => {
  //Create Operation used to add a todo
  db.collection("todos")
    .insertOne({ thing: request.body.todoItem, completed: false }) //inserts one document from the request
    .then((result) => {
...
app.put("/markComplete", (request, response) => {
  //Update Operation used to mark a todo complete
  db.collection("todos")
    .updateOne(
...
app.put("/markUnComplete", (request, response) => {
  //Update Operation to mark the the todo uncomplete
  db.collection("todos")
    .updateOne(
      { thing: request.body.itemFromJS },
      {
        //find the document todo using the todo text "itemFromJS" from the html form
        $set: {
...
app.delete("/deleteItem", (request, response) => {
  //Delete Operation to remove a todo from the DB
  db.collection("todos")
    .deleteOne({ thing: request.body.itemFromJS }) // find one todo based on itemFromJS (text from body) for deletion
    .then((result) => {
...
app.listen(process.env.PORT || PORT, () => {
  //Listen to the Port number from the environment or if that does not exist, use the port number from the variable in server.js
