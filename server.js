...
const MongoClient = require('mongodb').MongoClient //makes it possible to use methods associated with MongoClient and talks to our DB
const PORT = 2121 //setting a constant to define the location where our server will be listening
require('dotenv').config() //allows us to access inside of .env file
...
let db, //declare a variable called db but not assign a value
    ...
    dbName = 'todo' //declaring a variable and assigning the name of the datebase we will be using

MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) //creating a connection to MongoDB and passing in our connection string, also pssing in the additonal property
    .then(client => { //waiting for the connection and proceeding if successful and passing in all the client information
        ...
        db = client.db(dbName) //assigning a value to previously declared db variable that contaisn a db client factory method
    }) //closing our .then
    
//middleware
app.set('view engine', 'ejs') //sets the ejs as the default render method
...
app.get('/',async (request, response)=>{ //starts a GET method when the route route is passed in, sets up req and res parameters
    ...
    const itemsLeft = await db.collection('todos').countDocuments({completed: false}) //sets a variable and awaits a count of uncompleted items to late display in EJS
...
app.put('/markUnComplete', (request, response) => { //starts a PUT method when the markUnComplete route is passed in
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{ //look inside the db for the ONE item matching the name of the item passed in from the main.js file that was clicked on
        $set: { //starts a then if delete was successful
...
app.listen(process.env.PORT || PORT, ()=>{ //setting up a which port we will be listening on - either the port from the env. file of the port variable we set
