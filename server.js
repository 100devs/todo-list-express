...
app
  .get("/", async (request, response) => {
    //starts a GET method when teh root route is passed in, sets up req and res parameters
...
app.post("/addTodo", (request, response) => {
  // starts a POST method when the add (addTodo) route is passed in
  db.collection("todos")
    .insertOne({ thing: request.body.todoItem, completed: false }) //inserts a new item into todos collection, give a completed value of false by default
    .then((result) => {
      //if insert is successfull, do something
...
app.put("/markComplete", (request, response) => {
  ...
  db.collection("todos")
    ...
    .catch((error) => console.error(error))
    .then((result) => {
      //starts a then if the update was successful
...
app.put("/markUnComplete", (request, response) => {
  //starts a PUT method when the "markUnComplete" rout is passed in
  db.collection("todos")
    .updateOne(
      { thing: request.body.itemFromJS },
      {
    ...
    .then((result) => {
      //starts a then if update was successfull
...
app.delete("/deleteItem", (request, response) => {
  ...
  db.collection("todos")
    .deleteOne({ thing: request.body.itemFromJS }) //look inside teh todos collection for the ONE item that has a matching name for our JS file
    .then((result) => {
...
app.listen(process.env.PORT || PORT, () => {
  //setting up which port we will be listening on - either port from the .env file or the port variable we set
