...
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) //creating a connection to MongoDB, and passing in our connection string
    .then(client => { //waiting for the connection and proceeding if successful, and passing in all the client information
        ...
        db = client.db(dbName) //assigning a value to previously declared db variable that contatins a db client factory method
    }) //closing our .then
    
app.set('view engine', 'ejs') //sets ejs as the default render method
app.use(express.static('public')) //sets the location for static assets
app.use(express.urlencoded({ extended: true })) //Tells express to decode and encode URL's automatically. supports arrays and objects
...
app.post('/addTodo', (request, response) => { //starts a POST method when the add route is passed in
    ...
    .then(result => { //if insert is succesful, do something
        ...
        response.redirect('/') //redericts back to the homepage
...
app.put('/markComplete', (request, response) => { //starts a PUT method when the markComplete route is passed in
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{ //look in the db for one item matching the name of the item passed in from the main.js file that was clicked on
        ...
        upsert: false //prevents insertion if item does not arleady exist
    })
    .then(result => { //starts a then if update is succseful 
        console.log('Marked Complete') //logging the completion
...
app.delete('/deleteItem', (request, response) => { //starts a delete method when the delete route is passed
    ...
    .then(result => { //starts a then if delete is succesful
        console.log('Todo Deleted') //logging completion
...
app.listen(process.env.PORT || PORT, ()=>{ //setting up which port we will be listening on- either the port from the .env file or the port variable we set
    console.log(`Server running on port ${PORT}`) //console the running port
