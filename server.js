...
let db, // Declaring an empty 'db' variable
    dbConnectionStr = process.env.DB_STRING, // a connection string variable that gets the string from .env or heroku's variable
    dbName = 'todo' // Declaring the name of the database to 'dbName' variable
...
app.use(express.static('public'))
    // Tells express to decode and encode URLs automagically
...
app.get('/',async (request, response)=>{
    ...
    // Getting items with a 'complete' value of false
...
app.post('/addTodo', (request, response) => {
    ...
    // Console log error if they occur
    .catch(error => console.error(error))
})
...
app.put('/markUnComplete', (request, response) => {
    ...
    // Console logging that it's been marked uncomplete, and also responding back to the client in JSON, saying it's been marked uncomplete
    .then(result => {
        console.log('Marked Complete')
        response.json('Marked Complete')
    })
