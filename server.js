...
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }).then(
  (client) => {
    //Promise chain of all the collections in the cluster
...
app.get("/", async (request, response) => {
  ...
  const itemsLeft = await db
    .collection("todos")
    .countDocuments({ completed: false }); // Counts all documents which have not been completed and stores them in itemsLeft
...
app.post("/addTodo", async (request, response) => {
  // When user makes a post request to the /addTodo route...

  db.collection("todos")
    .insertOne({ thing: request.body.todoItem, completed: false }) // Insert a new todo document into the todos collection in the database, with the content of the todo determined by JSON data within the post request object.
    .then((result) => {
...
app.put("/markComplete", (request, response) => {
  // Put request! We are UPDATING/ALTERING a document which is already there
  db.collection("todos")
    .updateOne(
      //connect to todos collection, and update
...
app.delete("/deleteItem", (request, response) => {
  db.collection("todos")
    .deleteOne({ thing: request.body.itemFromJS }) // connect to todos collection and delete the document passed in from delete request
    .then((result) => {
      // once item deleted
...
app.listen(process.env.PORT || PORT, () => {
  // If runtime environment has a port specified, use it! Otherwise, use our own PORT variable's value
