...
let db, //declare a variable called db but not assign a vaule
...
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) //creating a connection to MongoDb, and passing in our connection string. Also passing in an additional property 
    .then(client => { // watiing for the connection and proceeding if successful, and passing in all the client information
        console.log(`Connected to ${dbName} Database`) // log to the console a template literal "connected to todo database"
        db = client.db(dbName) // assigning a vaule to the previously declared db variable that contains a db client factory method
    }) //closing out .then
    
app.set('view engine', 'ejs') //sets EJS as the default render method. View engines allow us to render web pages using EJS
...
app.get('/',async (request, response)=>{ // starts a GET method whrn the root route is passed in, sets up req ans res parameters
...
app.post('/addTodo', (request, response) => { // starts a POST method when the add route is passed in
    ...
    .then(result => { // if insert is successful, do something
        ...
        response.redirect('/') // gets rid os the /addTodo route, and redirects back to the homepage
    }) // closing the .then
...
app.put('/markComplete', (request, response) => { // starts a PUT method when the markComplete route is passed in
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{ // look in the db for one item matching the name of the item passed in from the main.js file that was clicked on
        $set: { // set completed status to true
...
app.delete('/deleteItem', (request, response) => { // starts a delete method when the route is passed
