...
MongoClient.connect(dbConnectionStr, { useUnifiedTopology: true }) //creating a connection to MongoDB, and passing in our connection string. Also passing in an additional property 
    .then(client => { //waiting for the connection and proceeding if successful, and passing in all client information
        ...
        db = client.db(dbName) //assigning a value to a previousy declared db variable that contains a db client factory method
    }) //closing our .then

//middleware
app.set('view engine', 'ejs') //sets ejs as the default method render method
app.use(express.static('public')) //sets the location for static assets
app.use(express.urlencoded({ extended: true })) //tells express to decode and encode URLs where the header matches the content. Supports arrays and objects.
app.use(express.json()) //parses JSON from incoming requests


app.get('/',async (request, response)=>{ //starts a GET when the root route is passed in, sets up req and res parameters
    ...
    response.render('index.ejs', { items: todoItems, left: itemsLeft }) //rendering the EJS file and passing through the db and the count remaining inside of an object
...
app.post('/addTodo', (request, response) => { //starts a POST method when the add route is passed in
    db.collection('todos').insertOne({thing: request.body.todoItem, completed: false}) //inserts a new item into todo collections, gives it a completed value of false by default.
...
app.delete('/deleteItem', (request, response) => { //starts a DELETE method when the add route is passed in
