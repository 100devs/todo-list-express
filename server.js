//declare variables to use our depencencies

//we use node & express to create and build our application
const express = require('express')//making it possible to use express in this file
//we declare our app as express
const app = express() //setting a constant and assigning it to the instance of express
//we declare our mongoDB connection to our database
const MongoClient = require('mongodb').MongoClient //makes it possible to use methods associated with MongoClient and talk to our DB
//we declare the local host port to access our app
const PORT = 2121 //setting a constant to defined the location where our server will be listening.
//we use .env to hide our mongodb password 
require('dotenv').config() //allows us to look for variables inside of the .env file

//mongoDB connection and database info database name
let db, //declaring a global variable called db but not assign a value
...
//middleware to view our site once host it
app.set('view engine', 'ejs') //sets ejs the default render method
app.use(express.static('public')) //sets the location for static assets
app.use(express.urlencoded({ extended: true })) //Tells express to decode and encode URLs where the header matches the content. Supports arrays and objects
app.use(express.json())//Parses JSON content from incoming requests

//we get data from our database
app.get('/',async (request, response)=>{ //starts a GET/READ method when the root route is passed in, sets up req and res parameters
    ...
    const itemsLeft = await db.collection('todos').countDocuments({completed: false})//sets a variable and awaits a count of uncompleted items to later disply in EJS
    response.render('index.ejs', { items: todoItems, left: itemsLeft })//rendering the ejs file and passing through the db items and the count remaining inside of an object
    // db.collection('todos').find().toArray()

    //another way to get data using .then/promises
    // .then(data => {
...
//we update data to our database
...
//listen help us connect to our local host port to view our app
