...
const item = document.querySelectorAll('.item span') // creating a variable and assinging it to a selection of all span tags within a parent that has a class of 'item'
const itemCompleted = document.querySelectorAll('.item span.completed') // creating a variable and assigning it to a selection of span tags with class 'completed' within a parent that has a class of 'completed'

Array.from(deleteBtn).forEach((element)=>{ // creating an array from our selection and starting a loop
    element.addEventListener('click', deleteItem) // add an event listener to the current item that waits for a click, then calls deleteItem function
}) // close loop

Array.from(item).forEach((element)=>{ // creating an array from our selection and starting a loop
    element.addEventListener('click', markComplete) // add an event listener to the current item that waits for a click, then calls markCoplete function
}) // close loop
...
async function markUnComplete(){ // declare an asychronous function
    const itemText = this.parentNode.childNodes[1].innerText // looks inside of the list item and grabs only the inner text within the list
    try{ // starting a try block to do something
