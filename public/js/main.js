//Client side variable declarations
const deleteBtn = document.querySelectorAll('.fa-trash') // create delete button variable that applies to each query with trash can icon
...
Array.from(deleteBtn).forEach((element)=>{ // create an array from deleteBtn
    element.addEventListener('click', deleteItem) // add a click even listener to each item
})

Array.from(item).forEach((element)=>{// create an array from item variable
    element.addEventListener('click', markComplete)// add a click even listener to each item
})

Array.from(itemCompleted).forEach((element)=>{// create an array from itemCompleted variable
    element.addEventListener('click', markUnComplete)// add a click even listener to each item
})

async function deleteItem(){// async function that deletes an item
    ...
    try{
        const response = await fetch('deleteItem', { // make fetch request and store in response variable
            method: 'delete', // method type, specifies it to be a delete
            headers: {'Content-Type': 'application/json'}, // header type
            body: JSON.stringify({ // parse the body object and convert it into JSON strings
              'itemFromJS': itemText // item being converted and sent via request to server
            })
          })
        const data = await response.json() // store the json response in data variable
        console.log(data) // logs data to console
        location.reload() // reload window to render items
...
async function markComplete(){ // function to mark item as complete
    ...
    try{
        ...
        console.log(data) // logs data to console
        location.reload() // reload window to render items. Item clicked will be now marked as complete (class applied via EJS)
...
async function markUnComplete(){// function to mark item as uncomplete
    ...
    try{
        const response = await fetch('markUnComplete', {// fetch request sent to markUnComplete route
            ...
            headers: {'Content-Type': 'application/json'}, // header type
            body: JSON.stringify({ // turn JS value into JSON object to be sent to the server
                'itemFromJS': itemText // item to be sent to mark uncomplete route
            })
          })
        const data = await response.json() // gets response from server as JSON
        console.log(data) // logs data to console
        location.reload() // reload window to render items. Item clicked will be now marked as uncomplete (class applied via EJS)
