...
async function deleteItem() { 
  ...
  try { //try block if promise is fulfilled
    const response = await fetch("deleteItem", { //uses FETCH API to delete todos
...
async function markComplete() { // allows for async/await syntax inside of markComplete() function
  const itemText = this.parentNode.childNodes[1].innerText; //assigns the todo to the itemText variable dynamically
  try {  // try block for if the promise is fulfilled
    const response = await fetch("markComplete", { //uses Fetch API to update task to completed
...
async function markUnComplete() {  // async function to mark a task an not complete
  const itemText = this.parentNode.childNodes[1].innerText;  //assigns the todo to the itemText variable dynamically
  try { // try block for if the promise is fulfilled
    const response = await fetch("markUnComplete", { //uses Fetch API to update task to incomplete
    ...
    location.reload();  // refreshes the page
  } catch (err) {  // catch block for if the promise is rejected
