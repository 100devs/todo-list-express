...
async function deleteItem(){//declaring an asynchronous function
    ...
    try{//starting a try block to do something
        const response = await fetch('deleteItem', {//creates a response variable that waits on a fetch to get data from the result of the deleteItem route
            ...
            body: JSON.stringify({//declare the message content being passedm and stringify that content 
              'itemFromJS': itemText //settiing the contemt of the body to the inner text of the list item, and naming it 'itemFromJs
            }) //closing the body
          }) //closing the object
...
async function markComplete(){ //declare an asynchronous function
    const itemText = this.parentNode.childNodes[1].innerText //looks inside of the list item and grabs only the inner text within the list span.
    try{ //starting a try block to do something
        const response = await fetch('markComplete', { //createsa  response variable that waits on a fetch to get data from the result of the markComplete route
