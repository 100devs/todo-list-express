...
/////////////// Event Listeners ////////////////////////////////
Array.from(deleteBtn).forEach((element)=>{ //Create an Array out of the deletBtn NodeList(which contains every delete button on the page)
...
Array.from(item).forEach((element)=>{ //Create an Array out of the item NodeList(which contains every item on the page)
...
Array.from(itemCompleted).forEach((element)=>{ //Create an Array out of the items completed NodeList(which contains every completed item on the page)
...
async function deleteItem(){ //a function that removes an item when trash can clicked
    ...
    }catch(err){ // Dodge this if no error
        console.log(err)
    }
}

async function markComplete(){ // A delighful function which will mark a task as complete, by going to markComplete route, which.... sets the complete property of the associated todo document to True, which, when EJS renders the task, applies the .completed task, which crosses the item out.
    
    const itemText = this.parentNode.childNodes[1].innerText // some janky ass dom node property accessing the text to grab 
    
    try{ // error prevention
        const response = await fetch('markComplete', { //fetch the markComplete route from server.js
