//create a variable deleteBtn Assign a NodeList of all HTMLElements with a class of `fa-trash`
const deleteBtn = document.querySelectorAll(".fa-trash");

//create a variable 'item' Assign a NodeList of all HTMLElements that are <span>s and descendants of elements with a class of `item`
...
async function deleteItem() {
  ...
  try {
    ...
    //if there is in error will pass to catch
  } catch (err) {
...
async function markComplete() {
  //it's this variable like previous one in deleteItem
  const itemText = this.parentNode.childNodes[1].innerText;
  try {
    // Make a fetch to the relative path `markComplete`
    //this is te same in previous function
