...
const itemCompleted = document.querySelectorAll('.item span.completed') //creating a variable and assigning it tot a selection of spans with a class of 'completed' inside of a parent with a class of 'item'
...
async function deleteItem(){ //declare an asynchronous function
    ...
    try{  //starting a stry block to do something
        const response = await fetch('deleteItem', { //creates a response variable that waits of a fetch to get data from the result of deleteItem route
    ...
    }catch(err){ //error handling. if an error occurs, pass the error into the catch block
        console.log(err) //log the error to the console
    } //close the catch block
} //end the function

async function markComplete(){ //delcare an asynchoronus function
    const itemText = this.parentNode.childNodes[1].innerText //looks inside of the list item angrabs only the inner text within the list span.
    try{ //starting a try block to do something
        const response = await fetch('markComplete', { //creates a response variable that waits on a fetch to get data from the result of the markComplete route
            method: 'put', //setting the CRUD method to 'update' fro the route
            headers: {'Content-Type': 'application/json'}, //specifying the type fo content expected, which is JSON
            body: JSON.stringify({ //declare the message content being passed, and stringify that content
                'itemFromJS': itemText //setting the content of the body to the inner text of the list item, and naming it 'interFromJS'
            }) //closing the body
          }) //closing the object
        const data = await response.json() //waiting on json from the response to be converted
...
async function markUnComplete(){ //delcare an asynchoronus function
    const itemText = this.parentNode.childNodes[1].innerText //looks inside of the list item angrabs only the inner text within the list span.
    try{ //starting a try block to do something
        ...
        const data = await response.json() //waiting on json fromt eh response to be converted
        console.log(data) //log the result to console
        location.reload() //reloads the page to update what is displayed
