const deleteBtn = document.querySelectorAll('.fa-trash')//creating a variable and     assiging it to a selection of all elements with a class of .fa-trash
const item = document.querySelectorAll('.item span')//creating a variable and assignign it to a selection of span tags inside of a parent that has a class of "item".
const itemCompleted = document.querySelectorAll('.item span.completed')//creating a variable and assignign it to a selection of spans with a class of completed inside of a parent with a class of item. 
...
Array.from(itemCompleted).forEach((element) => {//creating an array from our selection and starting a loop
    element.addEventListener('click', markUnComplete)//add an event listener to Only the completed items.
})//close our loop

async function deleteItem() {//declare an asynchronous function 
    const itemText = this.parentNode.childNodes[1].innerText//looks inside of the list item and grabs only the inner text within the list span.
    try {//starting a try block to do something
        const response = await fetch('deleteItem', {//creats a response variable that waits on a fetch to get data from the result of the deleteItem route.
...
async function markUnComplete() {//declare and asynchronous function
    const itemText = this.parentNode.childNodes[1].innerText//looks inside of the list item and grabs only the inner text within the list span.
    try {//starting a try block to do something
