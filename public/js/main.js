...
async function markComplete(){ //mark item as complete
    ...
    try{ //try to mark item as complete
        const response = await fetch('markComplete', { //send item text to server
...
async function markUnComplete(){ //mark item as incomplete
    const itemText = this.parentNode.childNodes[1].innerText //get item text from list item
    try{ //try to mark item as incomplete
        const response = await fetch('markUnComplete', { //send item text to server
            method: 'put', //send put request to server
            headers: {'Content-Type': 'application/json'}, //set content type to json
            body: JSON.stringify({ //send item text to server
                'itemFromJS': itemText //send item text to server
