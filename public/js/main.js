...
const item = document.querySelectorAll(".item span"); //creating a variable and assigning it to a selection of span tags inide of a parent that has a class if "item"
...
Array.from(deleteBtn).forEach((element) => {
  ...
  element.addEventListener("click", deleteItem); //add an event listener to the current item that waits for a click and then calls a functin called markComplete
}); //close our loop
...
async function deleteItem() {
  ...
  try {
    ...
    const response = await fetch("deleteItem", {
      ...
      headers: { "Content-Type": "application/json" }, //specifying the type of content expected, whic is JSON
      body: JSON.stringify({
...
async function markComplete() {
  ...
  try {
    ...
    const response = await fetch("markComplete", {
      ...
      body: JSON.stringify({
        //declare the message content being passed, and stringify that contant
        itemFromJS: itemText, //setting the content of the body to the inner text of the list item, and naming it "itemFromJS"
      }), //closing the body
    }); //closing the object
    const data = await response.json(); //waiting on JSON from the response to be converted
...
async function markUnComplete() {
  ...
  try {
    ...
    const response = await fetch("markUnComplete", {
      ...
      method: "put", //setting the CRUD method to "update" for the route
      headers: { "Content-Type": "application/json" }, //specifying the type of content expectdc, which is JSON
      body: JSON.stringify({
        //declare the message content being passed, and stringify that content
        itemFromJS: itemText, //setting the content of the body to the inner text of the list item and naming it 'itemFromJS'
      }), //closing the body
