const deleteBtn = document.querySelectorAll('.fa-trash') //creating a variable and assigning it to a selection of all ements with a class of the trash can
...
Array.from(deleteBtn).forEach((element)=>{ //creating an array from our selection and starting a loop
    element.addEventListener('click', deleteItem)  //add an event listener to the currrent item that waits for a click and then calls a function called deleteItem
})// close our loop
...
async function markComplete(){  //declare an asynchronus function
    const itemText = this.parentNode.childNodes[1].innerText  //looks inside of the list item and grabs only the inner text within the list span
    try{  //starting a try block to do something
        const response = await fetch('markComplete', {  //creates a response variable that waits on a fetch to get data from the result the markComplete route
...
async function markUnComplete(){  //declare an asynchronus function
    const itemText = this.parentNode.childNodes[1].innerText  //looks inside of hte list item and grabs only the inner text within the list span
    try{ // starting a try block to do something
