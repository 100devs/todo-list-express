...
async function deleteItem(){
    ...
    try{
        const response = await fetch('deleteItem', { // call the deleteItem Operation on the server
...
async function markComplete(){
    ...
    try{
        const response = await fetch('markComplete', { // call the markComplete Update Operation on the server
...
async function markUnComplete(){
    const itemText = this.parentNode.childNodes[1].innerText //grab the text from todo
    try{
        const response = await fetch('markUnComplete', { // call the markUnComplete Update Operation on the server
            method: 'put', //declare the type of operation that will be used
            headers: {'Content-Type': 'application/json'}, 
            body: JSON.stringify({
                'itemFromJS': itemText //add todo text to a json object called body
