...
Array.from(itemCompleted).forEach((element) => {
  //creating an array from our selection and staring a loop
  element.addEventListener("click", markUnComplete); //add an event listener to ONLY completed items
}); //close our loop

async function deleteItem() {
  //declaring asynchronous function
  const itemText = this.parentNode.childNodes[1].innerText; //looks inside of the list item and grabs only the inner text within the list span
  try {
    //starting a try block to do something
    const response = await fetch("deleteItem", {
      //creates a response variable that waits on a fetch to get data the result of the "deleteItem" route
...
async function markComplete() {
  //declaring and asynchronous function
  const itemText = this.parentNode.childNodes[1].innerText; //looks inside of the list item and grabs only the inner text within the list span
  try {
    //starting a try block to do something
    const response = await fetch("markComplete", {
      ...
      body: JSON.stringify({
        //delcare the message content being passes, and stringify that content
...
async function markUnComplete() {
  //decaring an asynchronous function
  const itemText = this.parentNode.childNodes[1].innerText; //looks inside of the list item and grabs only the inner text within the list span
  try {
    //starting a try block to do something
