...
const item = document.querySelectorAll('.item span') //assign to this var all span children of elements with class of item
...
Array.from(deleteBtn).forEach((element)=>{ //turn into array to use array method and efficiently add event listeners. Similar ethos as event delegation
    element.addEventListener('click', deleteItem) //call a particular function based on the element groupings
})
...
async function markComplete(){ //callback for when any elements in "items" array is clicked
...
async function markUnComplete(){ //callback associated with the itemCompleted array
    const itemText = this.parentNode.childNodes[1].innerText //as above
    try{
